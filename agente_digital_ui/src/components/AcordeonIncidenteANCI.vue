<template>
  <div class="acordeon-incidente-mejorado">
    ...
    <div class="header-con-navegacion">
      <button class="btn-volver" @click="$router.go(-1)">
              ...
              <div class="taxonomias-lista">
                ...
                >
                  ...
                  <div 
                    ...
                    class="taxonomia-campos-container"
                  >
                    ...
                    ">
                        ...
                        <div v-if="obtenerTaxonomiaSeleccionada(tax.id).archivos && obtenerTaxonomiaSeleccionada(tax.id).archivos.length > 0" style="margin-top: 15px;">
                            ...
                            <div v-if="obtenerTaxonomiaSeleccionada(tax.id).archivos.some(a => !a.existente)" 
                                 style="background: #fff3cd; color: #856404; padding: 10px; border-radius: 6px; margin-top: 10px; font-size: 13px;">
                              <i class="fas fa-info-circle"></i>
                              <strong>Nota:</strong> Los archivos nuevos se guardarán cuando guarde el incidente completo.
                            </div>
                        </div>
                        
                        <!-- BOTÓN GUARDAR TAXONOMÍA - REMOVIDO -->
                        <!-- Los cambios en taxonomías se guardan automáticamente en memoria 
                             y se aplicarán cuando guarde el incidente completo -->
                    </div>
                  </div>
                </div>
              </div>
...
async function cargarTaxonomias() {
  try {
    ...
    taxonomiasDisponibles.value = todasTaxonomias
      ...
      .map(tax => ({
        id: String(tax.id || tax.ID_Incidente || tax.Id_Incidente), // Normalizar a string
...
function taxonomiaSeleccionada(taxId) {
  // Normalizar a string para evitar problemas de tipos
...
function mostrarFormularioTaxonomia(taxId) {
  const idStr = String(taxId)
  const tax = taxonomiasSeleccionadas.value.find(t => String(t.id) === idStr)
...
function obtenerTaxonomiaSeleccionada(taxId) {
  const idStr = String(taxId)
  let tax = taxonomiasSeleccionadas.value.find(t => String(t.id) === idStr)
  if (!tax) {
...
function toggleTaxonomia(tax) {
  ...
  const taxIdStr = String(tax.id)
  const index = taxonomiasSeleccionadas.value.findIndex(t => String(t.id) === taxIdStr)
...
async function guardarTaxonomia(taxonomiaId) {
  ...
  // 1. MARCAR COMO GUARDADA EN EL ESTADO
  tax.guardada = true
...
async function cargarIncidenteExistente() {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/admin/incidentes/${props.incidenteId}`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    })
    ...
    if (datos.formData) {
      ...
      console.log('✅ FormData cargado desde formData:', Object.keys(formData.value).length, 'campos')
    } else {
      // Si no viene formData, mapear los campos directamente
      console.log('⚠️ No hay formData, mapeando campos directamente')
      formData.value = {
    ...
    if (datos.taxonomias_seleccionadas && Array.isArray(datos.taxonomias_seleccionadas)) {
      ...
      // Esperar a que las taxonomías disponibles se carguen
...
onMounted(async () => {
  ...
  if (props.datosIniciales) {
    ...
    if (props.datosIniciales.taxonomias_seleccionadas) {
      // Normalizar IDs a string al cargar
      taxonomiasSeleccionadas.value = props.datosIniciales.taxonomias_seleccionadas.map(tax => ({
        ...tax,
...
.taxonomia-item {
  ...
  position: relative !important; /* Para el indicador de checkmark */
...
/* ESTILOS PARA DESTACAR TAXONOMÍAS SELECCIONADAS */
...
.taxonomia-item.taxonomia-seleccionada .taxonomia-header {
...
/* Asegurar que los comentarios y archivos sean visibles */
.taxonomia-item.taxonomia-seleccionada .taxonomia-seleccionada > div {
    display: block !important;
    opacity: 1 !important;
}

/* Asegurar que el checkbox y el texto sean visibles en taxonomías seleccionadas */
...
.taxonomia-item.seleccionada::after {
    content: '✓';
    position: absolute;
    right: 1rem;
    top: 50%;
...
/* Contenedor de campos de taxonomía seleccionada */
.taxonomia-campos-container {
    margin-top: 1rem;
    padding: 1rem;
...
/* Asegurar que los archivos se muestren correctamente */
.taxonomia-campos-container .archivo-item {
    display: flex;
    align-items: center;
    padding: 0.75rem;
