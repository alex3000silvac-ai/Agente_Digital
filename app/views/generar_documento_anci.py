...
def generar_documento_anci(incidente_id):
    """
    Genera documentos ANCI en PDF, Word o TXT basados en datos del incidente semilla
    """
    try:
        ...
        formato = data.get('formato', 'pdf')  # pdf, word, txt
        ...
        if formato == 'pdf':
            documento = _generar_pdf(contenido, tipo_reporte, tipo_empresa)
            content_type = 'application/pdf'
            extension = 'pdf'
        elif formato == 'word':
            documento = _generar_word(contenido, tipo_reporte, tipo_empresa)
...
def _generar_pdf(contenido, tipo_reporte, tipo_empresa):
    ...
    story.append(Paragraph(contenido['titulo'], title_style))
    story.append(Spacer(1, 12))
    
    # Información de la empresa
    empresa_data = [
        ['Razón Social:', contenido['empresa']['razon_social']],
        ['RUT:', contenido['empresa']['rut']],
        ['Tipo Entidad:', contenido['empresa']['tipo_entidad']],
        ['Sector:', contenido['empresa']['sector']],
        ['Fecha Generación:', contenido['fecha_generacion']]
    ]
    ...
    for seccion in contenido['secciones']:
        story.append(Paragraph(seccion['titulo'], styles['Heading2']))
        story.append(Spacer(1, 12))
        ...
        if isinstance(seccion['contenido'], list):
            ...
            story.append(Paragraph(seccion['contenido'], styles['Normal']))
...
def _generar_word(contenido, tipo_reporte, tipo_empresa):
    """Genera documento Word"""
    doc = Document()
    
    # Título
    title = doc.add_heading(contenido['titulo'], 0)
    title.alignment = 1  # Centrado
    
    # Información de empresa
    doc.add_heading('INFORMACIÓN DE LA ENTIDAD', level=1)
